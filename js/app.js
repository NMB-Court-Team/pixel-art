(() => {
  ...
  async function exportPng(event){const scale=event?.altKey?1:16;if(typeof CompressionStream==='undefined'){exportPngCanvas(scale);return}try{const blob=new Blob([await buildIndexedPng(scale)],{type:'image/png'});downloadPng(blob,scale)}catch(error){console.warn('[Pixel Atelier] 索引色 PNG 导出失败，回退 canvas 导出',error);exportPngCanvas(scale)}}
