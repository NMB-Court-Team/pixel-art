(() => {
  ...
  // 最小色块格数取值：内置下限 16，最大 2304（整幅画布）；非法输入回退默认值
  ...
  function buildLabRestrictions(L,w,h,entries=displayPalette()){return entries.map(entry=>cacheValue(labRegionCache,`${state.minDeltaE}|${L}|${w}x${h}|${entrySignature(entry)}`,()=>{const paletteLab=colorLab(entry.color),center=entry.previewLab||paletteLab,blockedAt=(a,b)=>{const lab={L,a,b};if(!labInGamut(lab))return false;return deltaE00(actualLabFromLab(lab),paletteLab)<state.minDeltaE},d=radialContour(center.a,center.b,labRayLimit,blockedAt,(a,b)=>({x:(a+110)/220*w,y:(110-b)/220*h}));return d?{id:entry.id,color:entry.color,d}:null},160)).filter(Boolean)}
  ...
  function enforceMinColorPixels(palette,pixels,min=state.minColorPixels){
    ...
    [...dropped].sort((a,b)=>(counts.get(a.id)||0)-(counts.get(b.id)||0)||a.id.localeCompare(b.id)).forEach(entry=>{
      ...
      for(let i=0;i<pixels.length;i++){
        if(pixels[i]!==entry.id)continue;
        const x=i%SIZE,y=(i-x)/SIZE;
        for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){
          if(!dx&&!dy)continue;
          const nx=x+dx,ny=y+dy;
          if(nx<0||ny<0||nx>=SIZE||ny>=SIZE)continue;
          const neighbor=pixels[ny*SIZE+nx];
          if(neighbor!=null&&survivorIds.has(neighbor))border.set(neighbor,(border.get(neighbor)||0)+1);
        }
      }
      const lab=colorLab(entry.color);let best=null,bestBorder=-1,bestDistance=Infinity;
      border.forEach((length,id)=>{const distance=deltaE00(lab,labs.get(id));if(length>bestBorder||length===bestBorder&&distance<bestDistance-1e-9){best=id;bestBorder=length;bestDistance=distance}});
      if(!best)survivors.forEach(survivor=>{const distance=deltaE00(lab,labs.get(survivor.id));if(distance<bestDistance-1e-9||Math.abs(distance-bestDistance)<=1e-9&&(!best||survivor.id<best)){best=survivor.id;bestDistance=distance}});
      targets.set(entry.id,best);
    });
    const droppedIds=new Set(dropped.map(entry=>entry.id));
    return {palette:palette.filter(entry=>!droppedIds.has(entry.id)),pixels:pixels.map(id=>id!=null&&targets.has(id)?targets.get(id):id),mergedCount:dropped.length,dropped:[...droppedIds],fallback};
  }
  ...
  function renderPalette(){
    $('#studioPaletteCount').textContent=`${state.palette.length} / ${state.maxColors}`;$('#clearPalette').disabled=!state.palette.length;$('#paletteDeltaRule').textContent=state.minDeltaE;[['minDeltaE',state.minDeltaE],['importMinDeltaE',state.minDeltaE],['maxColors',state.maxColors],['importMaxColors',state.maxColors],['minColorPixels',state.minColorPixels],['importMinColorPixels',state.minColorPixels]].forEach(([id,value])=>{const input=$('#'+id);if(input&&document.activeElement!==input){input.value=value;input.classList.remove('invalid')}});renderStudioPalette();updateStatus();
  }
  ...
  const pointers=new Map(),importPointers=new Map();let panSession=null,pinchSession=null,suppressCanvasMenu=false;
  ...
  function drawLabView(){
    const c=$('#labCanvas'),ctx=c.getContext('2d'),w=c.width,h=c.height,L=+$('#labLightness').value,baseKey=String(L);if(c.dataset.baseKey!==baseKey){const base=cacheValue(labBaseCache,baseKey,()=>buildModelCanvas(LAB_MODEL,i=>labToRgb({L,a:LAB_MODEL.a[i],b:LAB_MODEL.b[i]})));ctx.clearRect(0,0,w,h);ctx.imageSmoothingEnabled=true;ctx.drawImage(base,0,0,w,h);c.dataset.baseKey=baseKey}const entries=displayPalette(),regionEntries=state.palette,restrictionKey=`${state.minDeltaE}|${L}|${paletteSignature(regionEntries)}`,regions=cacheValue(labRestrictionCache,restrictionKey,()=>buildLabRestrictions(L,w,h,regionEntries));renderRestrictionLayer($('#labRestrictionLayer'),restrictionKey,regions);const markers=entries.map((entry,i)=>{const lab=entry.previewLab||colorLab(entry.color);return Math.abs(lab.L-L)<=18?{x:(lab.a+110)/220*w,y:(110-lab.b)/220*h,color:entry.color,label:i.toString(16).toUpperCase(),active:entry.id===editingId}:null}).filter(Boolean);renderMarkerLayer($('#labMarkerLayer'),`${restrictionKey}|${paletteSignature(entries)}|${editingId||''}`,markers);const cursor=labCursor||rgbToLab(hexToRgb(candidate));updateCandidateMarker($('#labCandidateMarker'),(cursor.a+110)/220*w,(110-cursor.b)/220*h,cursor.a>=-110&&cursor.a<=110&&cursor.b>=-110&&cursor.b<=110&&!(colorDragPreview&&editingId))
  }
  function drawWheelView(){
    const c=$('#wheelCanvas'),ctx=c.getContext('2d'),w=c.width,h=c.height,cx=w/2,cy=h/2,R=Math.min(w,h)/2-3,L=+$('#wheelLightness').value,baseKey=String(L);if(c.dataset.baseKey!==baseKey){const base=cacheValue(wheelBaseCache,baseKey,()=>buildModelCanvas(WHEEL_MODEL,i=>WHEEL_MODEL.inside[i]?hslToRgb({h:WHEEL_MODEL.hue[i],s:WHEEL_MODEL.saturation[i],l:L}):null));ctx.clearRect(0,0,w,h);ctx.imageSmoothingEnabled=true;ctx.drawImage(base,0,0,w,h);c.dataset.baseKey=baseKey}const entries=displayPalette(),regionEntries=state.palette,restrictionKey=`${state.minDeltaE}|${L}|${paletteSignature(regionEntries)}`,regions=cacheValue(wheelRestrictionCache,restrictionKey,()=>buildWheelRestrictions(L,w,h,regionEntries));renderRestrictionLayer($('#wheelRestrictionLayer'),restrictionKey,regions);const markers=entries.map((entry,i)=>{const hsl=entry.previewHsl||rgbToHsl(hexToRgb(entry.color));if(Math.abs(hsl.l-L)>18)return null;const angle=hsl.h*Math.PI/180,r=hsl.s/100*R;return{x:cx+Math.cos(angle)*r,y:cy+Math.sin(angle)*r,color:entry.color,label:i.toString(16).toUpperCase(),active:entry.id===editingId}}).filter(Boolean);renderMarkerLayer($('#wheelMarkerLayer'),`${restrictionKey}|${paletteSignature(entries)}|${editingId||''}`,markers);const cursor=wheelCursor||rgbToHsl(hexToRgb(candidate)),angle=cursor.h*Math.PI/180,r=cursor.s/100*R;updateCandidateMarker($('#wheelCandidateMarker'),cx+Math.cos(angle)*r,cy+Math.sin(angle)*r,cursor.s>=0&&cursor.s<=100&&!(colorDragPreview&&editingId))
  }
  ...
  function enforcePaletteSeparation(samples,centers){
    ...
    for(let i=0;i<clusters.length;i++)for(let j=i+1;j<clusters.length;j++){const distance=deltaE00(clusters[i].rep.lab,clusters[j].rep.lab);if(distance<state.minDeltaE)throw new Error(`调色板距离修复失败：色差 ${distance.toFixed(3)}`)}return {clusters,mergedCount:before-clusters.length,sources:[...unique.values()]}
  }
  ...
  function quantizeImage(data,{algorithm='none',maxOffset=8,contrastTarget=12,contrastStrength=65,regionThreshold=3}={}){
    const samples=[],visible=[];for(let i=0;i<SIZE*SIZE;i++){const alpha=data[i*4+3];if(alpha<32)continue;const rgb={r:data[i*4],g:data[i*4+1],b:data[i*4+2]},lab=rgbToLab(rgb),weight=alpha/255;samples.push({i,rgb,lab,weight});visible.push({rgb,lab,weight})}if(!samples.length)return {palette:[],pixels:Array(SIZE*SIZE).fill(null),mergedCount:0,smallStats:{merged:0,fallback:false}};
  ...
  function decodeFloorPattern(item){
    ...
    if(colors.length>state.maxColors)throw new Error(`颜色数量 ${colors.length} 超过调色板上限 ${state.maxColors}`);
    ...
    return {name:item.name,colors,palette:small.palette,pixels:small.pixels,opaque:pixels.filter(Boolean).length,smallMerged:small.mergedCount,smallFallback:small.fallback};
  }
  ...
  function bindEvents(){
    ...
    const minPixelInputs=[$('#minColorPixels'),$('#importMinColorPixels')],minPixelsDraftValid=input=>{const value=+input.value,valid=Number.isFinite(value)&&Number.isInteger(value)&&value>=MIN_COLOR_PIXELS&&value<=MAX_MIN_COLOR_PIXELS;input.classList.toggle('invalid',!valid);return valid},syncMinPixelInputs=()=>minPixelInputs.forEach(input=>{if(document.activeElement!==input){input.value=state.minColorPixels;input.classList.remove('invalid')}}),commitMinColorPixels=input=>{if(!minPixelsDraftValid(input)){toast(`最小色块不能低于内置值 ${MIN_COLOR_PIXELS}，且不能超过 ${MAX_MIN_COLOR_PIXELS}`);setTimeout(()=>{input.value=state.minColorPixels;input.classList.remove('invalid')},450);return}const value=Math.round(+input.value);if(value===state.minColorPixels){syncMinPixelInputs();return}const refreshPreview=!!importState?.preview;beginAction('调整最小色块格数');state.minColorPixels=value;commitAction();if(importState)importState.result=null;renderPalette();syncMinPixelInputs();if(importState)syncImportControls();if(refreshPreview)refreshImportPreview();const shortCount=shortColorEntries().length;toast(`最小色块已设为 ${value} 格${shortCount?`；当前有 ${shortCount} 个颜色不足，可点「整理」合并`:''}`)};
    ...
    const finishColorInteraction=(e,isClick)=>{colorDragPreview=false;redrawActiveColorMap();if(!isClick||e.shiftKey===e.altKey)return;if(e.shiftKey){requestAddCandidate();return}if(!editingId){toast('请先从调色板选择要更新的颜色');return}if(!applyCandidate('update'))toast(`与已有颜色过近，至少需要色差 ${state.minDeltaE}`)};
    ...
    const applyImportScale=()=>{$('#importScale').value=Math.round(importState.multiplier*100);$('#importScale').nextElementSibling.textContent=Math.round(importState.multiplier*100)+'%'};
    const startImportPinch=()=>{const pinch=importPinchState();if(!importState||!pinch)return;const mid=previewUnits(pinch.cx,pinch.cy);importState.drag=null;importState.pinch={dist:pinch.dist,multiplier:importState.multiplier,midX:mid.x,midY:mid.y,centerX:importState.x,centerY:importState.y}};
    const moveImportPinch=()=>{const pinch=importPinchState(),session=importState?.pinch;if(!pinch||!session)return;const mid=previewUnits(pinch.cx,pinch.cy),next=clamp(session.multiplier*pinch.dist/session.dist,.1,5),k=next/session.multiplier;importState.multiplier=next;importState.x=session.midX+(session.centerX-session.midX)*k+(mid.x-session.midX);importState.y=session.midY+(session.centerY-session.midY)*k+(mid.y-session.midY);applyImportScale();drawImportPreview()};
    const endImportPointer=e=>{importPointers.delete(e.pointerId);if(!importState)return;if(importPointers.size<2)importState.pinch=null;const [only]=[...importPointers.values()];if(importPointers.size===1&&only)importState.drag={x:only.x,y:only.y,ox:importState.x,oy:importState.y};if(!importPointers.size)importState.drag=null};
    $('#importPreview').onpointerdown=e=>{if(!importState)return;e.preventDefault();importPointers.set(e.pointerId,{x:e.clientX,y:e.clientY});e.currentTarget.setPointerCapture?.(e.pointerId);leaveImportPreview();if(importPointers.size>=2){startImportPinch();drawImportPreview();return}importState.drag={x:e.clientX,y:e.clientY,ox:importState.x,oy:importState.y};drawImportPreview()};
    $('#importPreview').onpointermove=e=>{if(!importState)return;if(importPointers.has(e.pointerId))importPointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(importState.pinch&&importPointers.size>=2){moveImportPinch();return}if(!importState.drag)return;const r=e.currentTarget.getBoundingClientRect();importState.x=importState.drag.ox+(e.clientX-importState.drag.x)*450/r.width;importState.y=importState.drag.oy+(e.clientY-importState.drag.y)*450/r.height;drawImportPreview()};
    $('#importPreview').onpointerup=endImportPointer;$('#importPreview').onpointercancel=endImportPointer;
    $('#importPreview').onwheel=e=>{e.preventDefault();if(!importState)return;leaveImportPreview();importState.multiplier=clamp(importState.multiplier*(e.deltaY<0?1.08:.92),.1,5);applyImportScale();drawImportPreview()};
