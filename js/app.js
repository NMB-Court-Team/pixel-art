(() => {
  ...
  const makeDefaultState=()=>({palette:DEFAULT_COLORS.map((color,i)=>({id:`p${i}`,color})),pixels:Array(SIZE*SIZE).fill(null),currentId:null,tool:'pen',symmetry:'none',grid:false,minDeltaE:DEFAULT_MIN_DELTA_E,maxColors:MAX_COLORS,penBrushShape:'square',penBrushRadius:0,eraserBrushShape:'square',eraserBrushRadius:0});
  ...
  function projectJson(){return JSON.stringify({format:'pixel-atelier',version:2,size:SIZE,minDeltaE:state.minDeltaE,maxColors:state.maxColors,palette:state.palette,pixels:state.pixels},null,2)}
