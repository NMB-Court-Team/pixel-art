(() => {
  ...
  function bindEvents(){
    ...
    const finishColorInteraction=(e,isClick)=>{colorDragPreview=false;redrawActiveColorMap();if(!isClick||e.shiftKey===e.altKey)return;if(e.shiftKey){requestAddCandidate();return}if(!editingId){toast('请先从调色板选择要更新的颜色');return}if(!applyCandidate('update'))toast(`与已有颜色过近，至少需要 色差 ${state.minDeltaE}`)};
